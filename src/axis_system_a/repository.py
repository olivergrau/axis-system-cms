"""Persistence layer and repository model for the Experimentation Framework.

Provides deterministic filesystem-based artifact storage:
- Directory layout resolution for experiments, runs, and episodes
- Save/load for all artifact types (configs, results, summaries, metadata, status)
- Immutable write semantics for result artifacts
- Discovery helpers for listing experiments, runs, and episode files
"""
...
# ---------------------------------------------------------------------------
# Status record wrappers (for JSON serialization)
# ---------------------------------------------------------------------------
...
class ExperimentRepository:
    ...
    def run_log_path(self, experiment_id: str, run_id: str) -> Path:
        return self.run_dir(experiment_id, run_id) / "logs" / "run.log.jsonl"
    ...
    def save_episode_result(
        self, experiment_id: str, run_id: str, episode_index: int,
        result: EpisodeResult, *, overwrite: bool = False,
    ) -> Path:
    ...
    def load_episode_result(
        ...
        return EpisodeResult.model_validate(
