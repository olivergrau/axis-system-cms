...
def render_series_measurement_plots(
    ...
    *,
    ...
    entries = tuple(payload.get("experiments", ()))
    if not entries:
        raise ValueError(
    ...
    for entry in entries:
        ...
        if not comparison_path.is_file():
            ...
            continue
        comparison_envelope = json.loads(comparison_path.read_text(encoding="utf-8"))
        comparison_result = comparison_envelope["comparison_result"]
...
def _build_plots_report_markdown(
    *,
    ...
    experiment_ids = sorted(
        {
            Path(item.relative_output_path).parts[-4]
            for item in generated_sorted
    ...
    if experiment_ids:
        ...
        for experiment_id in experiment_ids:
            ...
            for item in generated_sorted:
                if item.plot_group != "experiment_comparison":
                    continue
                parts = Path(item.relative_output_path).parts
                if len(parts) >= 4 and parts[-4] == experiment_id:
                    add_plot_item(item)
...
def _render_series_progression(*, ws: Path, workspace_type: str, entries, output_root: Path):
    ...
    cand = [entry["comparison_summary"]["candidate_survival_rate"] for entry in entries]
    ref = [entry["comparison_summary"]["reference_survival_rate"] for entry in entries]
    fig = create_figure(figsize=(10, 7))[0]
    ax1 = fig.add_subplot(2, 1, 1)
    ax2 = fig.add_subplot(2, 1, 2)
    ax1.plot(xs, death, marker="o", label="death rate")
    ax1.plot(xs, vitality, marker="o", label="mean final vitality")
    ax1.plot(xs, efficiency, marker="o", label="net energy efficiency")
    ax1.set_xticks(xs, labels, rotation=30, ha="right")
    ax1.legend()
    cand_label, ref_label = _survival_label_pair(workspace_type)
    ax2.plot(xs, cand, marker="o", label=cand_label)
    ax2.plot(xs, ref, marker="o", label=ref_label)
    ax2.set_xticks(xs, labels, rotation=30, ha="right")
    ...
    return GeneratedPlotArtifact(
        ...
        description="Tracks key run and comparison metrics over experiment order.",
        producer_kind="framework",
    )
