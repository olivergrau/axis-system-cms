...
- `docs/v0.2.0/architecture/evolution/architectural-vision-v0.2.0.md` -- Section 6 (Experimentation Framework)
- `docs/v0.2.0/architecture/evolution/modular-architecture-roadmap.md` -- WP-1.4 definition
- `docs/v0.2.0/architecture/evolution/modular-architecture-questions-answers.md` -- Q6, Q7, Q12
...
- Any modifications outside `src/axis/framework/`, `src/axis/sdk/`, and `tests/v02/`
...
tests/v02/framework/test_config.py          # NEW (verification tests)
tests/v02/builders/config_builder.py        # MODIFIED (updated to use FrameworkConfig)
tests/v02/builders/system_config_builder.py # UNCHANGED (still produces dicts)
tests/v02/constants.py                      # UNCHANGED
```
...
### Config type verification tests (`tests/v02/framework/test_config.py`)
...
10. **Constants consistency**:
    - Default values in `FrameworkConfig` built from `FrameworkConfigBuilder().build()` match the constants in `tests/v02/constants.py`
...
3. Updated `tests/v02/builders/config_builder.py` to produce `FrameworkConfig`
4. Updated `tests/v02/fixtures/config_fixtures.py` with typed fixtures
5. Verification tests at `tests/v02/framework/test_config.py`
6. Updated `tests/v02/test_infrastructure.py` if needed (builder return type changed)
...
The most important property of the config types is **consistency with the existing test fixtures**. The default values in `FrameworkConfig` (built via `FrameworkConfigBuilder`) must match `tests/v02/constants.py`, which in turn matches the v0.1.0 `_BASE_CONFIG_DICT` from `tests/fixtures/scenario_fixtures.py`. When Phase 2 runs behavioral equivalence tests, these defaults ensure numerical identity.
