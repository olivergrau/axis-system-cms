...
- `docs/v0.2.0/architecture/evolution/architectural-vision-v0.2.0.md` -- Section 3 (package structure)
- `docs/v0.2.0/architecture/evolution/modular-architecture-roadmap.md` -- WP-0.2 definition
- `docs/v0.2.0/architecture/evolution/modular-architecture-questions-answers.md` -- Q18
...
Create a documented builder convention under `tests/v02/builders/`.

**File**: `tests/v02/builders/__init__.py`
...
**File**: `tests/v02/builders/config_builder.py`
...
**File**: `tests/v02/builders/system_config_builder.py`
...
Create assertion helper modules under `tests/v02/utils/`.

**File**: `tests/v02/utils/__init__.py`
...
**File**: `tests/v02/utils/assertions.py`
...
**File**: `tests/v02/utils/trace_assertions.py`
...
**File**: `tests/v02/fixtures/__init__.py`
...
**File**: `tests/v02/fixtures/config_fixtures.py`
...
**File**: `tests/v02/test_infrastructure.py`
...
No existing test file under `tests/` (outside of `tests/v02/`) is modified. The existing `conftest.py` at `tests/conftest.py` is not changed. Both old and new test trees must run successfully under `pytest tests/`.
...
- Modifications to any file outside `tests/v02/`
...
The `tests/v02/` tree has its own `conftest.py` and does not inherit fixtures from the root `tests/conftest.py`. This prevents accidental dependencies on `axis_system_a`-specific fixtures in the new test suites.

If a test under `tests/v02/` needs a fixture from the existing tree, it must be explicitly re-created or imported -- not silently inherited.
...
```
tests/v02/conftest.py                           # MODIFIED (was empty, now has pytest_plugins)
tests/v02/constants.py                          # NEW
tests/v02/builders/__init__.py                  # NEW (docstring + convention docs)
tests/v02/builders/config_builder.py            # NEW (FrameworkConfigBuilder)
tests/v02/builders/system_config_builder.py     # NEW (SystemAConfigBuilder)
tests/v02/fixtures/__init__.py                  # NEW (docstring only)
tests/v02/fixtures/config_fixtures.py           # NEW (config fixtures)
tests/v02/utils/__init__.py                     # NEW (docstring only)
tests/v02/utils/assertions.py                   # NEW (generic assertion helpers)
tests/v02/utils/trace_assertions.py             # NEW (placeholder)
tests/v02/test_infrastructure.py                # NEW (verification tests)
```
...
tests/v02/test_scaffold.py                      # UNCHANGED (from WP-0.1)
tests/v02/sdk/, tests/v02/framework/, etc.      # UNCHANGED (empty from WP-0.1)
```
...
### Infrastructure verification tests (`tests/v02/test_infrastructure.py`)
...
1. Builder modules under `tests/v02/builders/`
2. Assertion helper modules under `tests/v02/utils/`
3. Fixture module under `tests/v02/fixtures/`
4. Constants module at `tests/v02/constants.py`
5. Updated `tests/v02/conftest.py` with fixture registration
6. Verification tests at `tests/v02/test_infrastructure.py`
