...
- `docs/v0.2.0/architecture/evolution/visualization-architecture.md` -- Section 4 (WorldVisualizationAdapter), Section 5 (SystemVisualizationAdapter)
- `docs/v0.2.0/architecture/evolution/visualization-architecture-roadmap.md` -- WP-V.1.2
...
**File**: `tests/v02/visualization/test_protocols.py` (new)
...
The mock adapters created for these tests should be reusable by later WPs (WP-V.1.4 registry tests, WP-V.3.4 view model builder tests). Consider placing them in a shared test fixtures module `tests/v02/visualization/conftest.py` or `tests/v02/builders/mock_adapters.py`, but only if convenient. At minimum they must exist in the test file.
...
2. `tests/v02/visualization/test_protocols.py` with protocol conformance tests
