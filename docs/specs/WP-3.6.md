...
- `docs/architecture/evolution/modular-architecture-roadmap.md` -- WP-3.6 definition
...
### 1. Mock System (`tests/framework/mock_system.py`)
...
#### `tests/framework/test_mock_system.py` -- Mock System Validation
...
#### `tests/framework/test_framework_integration.py` -- Cross-Component Integration
...
#### `tests/framework/test_ofat_integration.py` -- OFAT Path Resolution End-to-End
...
#### `tests/framework/test_error_handling.py` -- Error Paths and Edge Cases
...
#### `tests/framework/test_system_a_through_framework.py` -- System A via Framework Pipeline
...
**Implementation approach**: These tests reuse the equivalence test helpers from `tests/systems/system_a/test_equivalence.py` (specifically `_run_legacy_episode()` and the `Trajectory` dataclass) to compare framework runner output against the legacy oracle.
...
```
tests/framework/mock_system.py                    # NEW (MockSystem implementation)
tests/framework/test_mock_system.py                # NEW (mock system validation)
tests/framework/test_framework_integration.py      # NEW (cross-component integration)
tests/framework/test_ofat_integration.py           # NEW (OFAT end-to-end)
tests/framework/test_error_handling.py             # NEW (error paths)
tests/framework/test_system_a_through_framework.py # NEW (System A via framework)
```
...
1. Mock system at `tests/framework/mock_system.py`
2. Mock system tests at `tests/framework/test_mock_system.py`
3. Integration tests at `tests/framework/test_framework_integration.py`
4. OFAT integration tests at `tests/framework/test_ofat_integration.py`
5. Error handling tests at `tests/framework/test_error_handling.py`
6. System A through-framework tests at `tests/framework/test_system_a_through_framework.py`
